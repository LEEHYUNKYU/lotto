...
if (!empty($_GET['promote']) && !in_array($_GET['promote'], array('Q', 'R', 'N', 'B'))) {
    $_GET['promote'] = 'Q';
}
/**
...
//        echo '<tr><td>&nbsp;</td><td colspan="2">White</td><td colspan="2">Black</td></tr>';
    ...
    function domove()
    {
        if (!empty($_GET['from']) && !empty($_GET['to']) && !empty($_GET['promote'])) {
            $err = $this->_board->moveSquare($_GET['from'], $_GET['to'], $_GET['promote']);
            if ($this->_board->isError($err)) {
                echo '<b>' .$err->getMessage() . '</b><br />';
            }
            $this->promote = false;
        ...
        foreach($board as $square => $piece) {
            ...
            if ($piece) {
                if (file_exists($imagepath . '/' . strtoupper($piece) . '.gif')) {
                    ...
                    $imgsize = GetImageSize($imagepath . DIRECTORY_SEPARATOR . $image);
                    echo '<img src="'. $imagepath . $image . '" border="0" width="' . $imgsize[0] . '" height="' . $imgsize[1] . '" alt="'. $piece . '">';
                } else {
                    echo $piece;
                }
            } else {
...
if (!isset($x) || isset($_GET['newgame']) || $fen || isset($_GET['newlosergame']) || isset($_GET['newcrazyhousegame'])) {
